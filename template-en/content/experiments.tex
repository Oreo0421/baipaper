...
This chapter presents a comprehensive experimental evaluation of the proposed thermal infrared fall detection system. We first describe the dataset and evaluation protocol (Sections~\ref{sec:dataset}--\ref{sec:metrics}), followed by implementation details (Section~\ref{sec:implementation}). We then conduct a series of ablation studies to validate our design choices: pose estimation representation (Section~\ref{sec:ablation_pose}), fall classification strategy (Section~\ref{sec:ablation_fall}), loss function design (Section~\ref{sec:ablation_loss}), and input feature representation (Section~\ref{sec:ablation_feature}). Finally, we present the full system comparison (Section~\ref{sec:full_system}), qualitative results (Section~\ref{sec:qualitative}), computational cost analysis (Section~\ref{sec:cost}), and a consolidated discussion (Section~\ref{sec:discussion}).

\section{Dataset}
\label{sec:dataset}

The thermal infrared fall detection dataset used in this study was collected using a Heimann HTPA 60$\times$40 thermopile sensor array within the SensIR project. The dual-sensor configuration yields a combined input resolution of $114 \times 40$ pixels, resized to $384 \times 384$ for training and inference. Annotations follow the COCO format and include: (i) bounding boxes for a single ``human'' class in $[x, y, w, h]$ format; (ii) seven keypoints per person---Head (0), Shoulder center (1), Right hand (2), Left hand (3), Hips center (4), Right foot (5), and Left foot (6)---each with a three-level visibility flag (0 = out-of-frame/unlabeled, 1 = occluded, 2 = visible); (iii) a ten-class action label per instance; and (iv) pre-computed temporal keypoint sequences of $T = 8$ frames with normalized keypoint features (five dimensions per keypoint: $x$, $y$, visibility, $\Delta x$, $\Delta y$).
...
\end{table}
All frame-level metrics are reported on the validation split. Additionally, a separate sequence-level test set comprising 38 video sequences (18 fall, 20 non-fall) from six activity scenarios is used to evaluate end-to-end detection performance at the event level.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\columnwidth]{figures/all.pdf}
  \caption[Qualitative thermal IR examples]{Qualitative examples of thermal IR sequences. Columns indicate pose categories}
  \label{fig:qual_grid}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\columnwidth]{figures/42.pdf}
  \caption[examples illustrating the visual ambiguity]{Qualitative examples illustrating the visual ambiguity between lying and falling in thermal infrared sequences.}
  \label{fig:qual_grid}
\end{figure}
...
SimDR~\cite{li20212d} reformulates 2D keypoint localization as two independent 1D classification tasks along the $x$- and $y$-axes, using KL divergence against 1D Gaussian targets ($\sigma = 1.5$) and a scale factor $s = 2$ yielding 96 bins per axis for sub-pixel precision. The full architecture and loss formulation are described in Section~\ref{sec:pose_head}.
...
\begin{figure}[t]
  ...
  \caption[Qualitative comparison of pose heads]{Qualitative comparison of pose heads on thermal IR examples. Each row shows one example; each image contains three columns: SimDR, Heatmap, and Regression.}
...
The proposed design is described in full in Section~\ref{sec:action_head}. In brief, it employs a shared GRU temporal encoder with two output heads: a \textbf{four-class posture head} (standing, walking, sitting, lying) trained with label-smoothed cross-entropy, and a \textbf{binary fall head} trained with focal BCE and hard negative weighting ($3\times$ for confusable Transition/Lying classes). The fall label is derived independently from the activity label: annotation classes 6--9 map to $y_\text{fall}=1$, classes 0--5 to $y_\text{fall}=0$.

\subsection{Single-Task Binary Fall Detection (Ablation)}
\label{sec:singletask}

This variant focuses all model capacity on binary fall detection, using the same GRU encoder but with a single binary classifier head and no posture branch. It retains the same focal loss, hard negative mining, and per-keypoint weighting as the proposed method. The loss is:
\begin{equation}
...
\end{equation}
All other hyperparameters are identical to those of the proposed method.
...
Both methods were fundamentally constrained by the dataset size. With only 4{,}838 training instances, the dual-task model's higher capacity could not be fully utilized. The effect of supplementing training with synthetic data provided by a colleague within the SensIR project is reported in Section~\ref{sec:full_system}.

  \begin{figure}[t]
      \centering
      \includegraphics[width=\linewidth]{figures/fig3_confusion_matrix.pdf}
      \caption[Confusion matrices of the dual-task method]{Confusion matrices of the proposed dual-task method.
...
\begin{table}[t]
\centering
\caption{Complete system comparison. bbox mAP and Kpt AP are evaluated on the frozen Stage~1 checkpoint integrated into the full pipeline and therefore differ from the standalone Stage~1 results in Table~\ref{tab:pose_comparison}.}
...
Prop.+Synth. & SimDR & 4-cls+bin. & Kpt (35d) & \textbf{0.776} & 0.951  & \textbf{0.834} & 0.931 & 0.734 \\
\hline
\end{tabular}%
}
\end{table}
...
Third, \textbf{the proposed dual-task method achieved the highest precision} (1.000) among all methods, producing zero false positives on the validation set. While high precision reduces alarm fatigue for care staff, it is important to note that in clinical fall detection, false negatives are the more serious failure mode: a missed fall leaves the patient without assistance, whereas a false alarm can be cancelled through a voice confirmation step. The lower recall (0.502) compared to the Binary-BCE baseline (0.629) therefore represents the primary limitation of the dual-task model on the current dataset, reflecting the inherent challenge of jointly optimizing posture classification and fall detection with limited training data. The Binary-BCE baseline achieved the highest Fall F1 (0.748) by concentrating all model capacity on a single task with a simple loss---a well-established advantage of parsimonious models on small datasets.

Fourth, \textbf{supplementing with synthetic data} improves the proposed architecture's performance substantially. Additional synthetic training samples were provided by a colleague within the SensIR project. Training the proposed dual-task model with this supplementary data (row ``Prop.+Synth.'') raises Fall F1 from 0.669 to 0.776 (+16\% relative) and recall from 0.502 to 0.655, surpassing the Binary-BCE baseline (F1: 0.748). This confirms that the dual-task architecture's higher representational capacity is underutilized on the original dataset alone and scales favorably with additional data.

\section{Qualitative Results}
\label{sec:qualitative}

The system correctly handled the majority of common indoor monitoring scenarios, with fall detection typically triggering within 2--3 frames ($\sim$0.2--0.3 seconds at 10~FPS) of the fall onset. For standard walking and standing activities, the model consistently produced near-zero fall probabilities, demonstrating effective discrimination between routine activities and genuine falls. The proposed method's perfect precision meant that no false alarms were generated in this evaluation; in practice, however, the missed falls (low recall) represent the more critical risk, as undetected falls cannot be caught by downstream confirmation mechanisms.
...
\textbf{Dual-task learning provides the most architecturally complete system.} While the simpler Binary-BCE baseline achieved the highest Fall F1 (0.748), the proposed dual-task method offered a more architecturally sound solution by: (a) providing clinically valuable posture classification output that single-task approaches entirely lack; (b) achieving perfect fall precision (1.000), reducing alarm fatigue for care staff (though at the cost of lower recall, which remains the primary clinical concern since false negatives are more dangerous than false positives in fall monitoring); and (c) possessing higher representational capacity that will benefit from additional training data. The performance gap was a direct consequence of the limited dataset size (4{,}838 instances) rather than an architectural limitation---a conclusion supported by the results obtained with synthetic data provided by a colleague.

\textbf{Simple approaches excel on small data, but complex architectures scale better.} The consistent ranking across our experiments---Binary-BCE $>$ Binary-Focal $>$ Dual-task on Fall F1---reflects the well-known bias-variance trade-off. Models with fewer optimization objectives and simpler loss functions generalize more readily with limited training examples. However, this advantage is expected to diminish as training data grows, at which point the richer representations learned by the dual-task architecture should yield superior performance. Indeed, supplementing training with synthetic data provided by a colleague raises the proposed dual-task method's Fall F1 to 0.776, surpassing the Binary-BCE baseline (0.748) and confirming that the architecture's higher representational capacity becomes advantageous with sufficient training data.
