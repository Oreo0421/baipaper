Term papers and theses are prepared in DIN~A4 format ($\unit[210 \times 297]{mm}$).
The paper format is already predefined by the class option \texttt{a4paper} in the
template (see Listing~\ref{lst:tpl:thesis}).
...
publications usually use the DIN~A5 format ($\unit[210 \times 297]{mm}$).
Authors of such works now face a problem. Reviewers usually require the work in
DIN~A4 format, but the publication takes place in a different format.
There are two possible solutions, both of which have their advantages and
disadvantages.
...
basic size for the A4~template at least the basic font size~\unit[12]{pt}. When
scaling from A4 to A5, the document will be reduced to approx. A character of
size~\unit[12]{pt} is then only barely \unit[9]{pt} in size. However, \LaTeX\
uses a separate font for each font size. A character reduced from \unit[12]{pt}
to \unit[9]{pt} has a different appearance than a character that is used for
\unit[9]{pt}, designed for \unit[9]{pt}. When scaling down, the lines are
lines are often thinner than in the respective design size. The font
"`Latin Modern"' is particularly susceptible to this
(see section~\ref{sec:tpl:font}).
